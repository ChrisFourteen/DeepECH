...
# TimeSeriesAttnLSTM implementation
...
class TranPosSpace(nn.Module):
    """
    Experiment: transformer_kanpos_3407
    """
    def __init__(self, dropout_rate = 0.1):
        super(TranPosSpace, self).__init__()
        self.transformer1 = TimeSeriesTransformer(seq_length=49)  # For columns 5-53
        self.transformer2 = TimeSeriesTransformer(seq_length=49)  # For columns 54-102
        self.fc1 = nn.Linear(128 + 500, 1024)  # Two transformer outputs and first 4 columns
    ...
    def forward(self, x):
        ts_features1 = self.transformer1(x[:, 4:53])  # Feature extraction (cols 5-53)
        ts_features2 = self.transformer2(x[:, 53:102])  # Feature extraction (cols 54-102)
...
class LstmPosSpace(nn.Module):
    ...
    def __init__(self, dropout_rate=0.1):
        ...
        self.lstm1 = TimeSeriesLSTM(hidden_dim=64)  # Output dim 64
        self.lstm2 = TimeSeriesLSTM(hidden_dim=64)  # Output dim 64
        self.pos_fc1 = nn.Linear(4, 100)
        self.pos_fc2 = nn.Linear(100, 200)
        self.pos_fc3 = nn.Linear(200, 500)
    ...
    def forward(self, x):
        ts_features1 = self.lstm1(x[:, 4:53].unsqueeze(-1))  # Ensure correct dimensions
        ts_features2 = self.lstm2(x[:, 53:102].unsqueeze(-1))  # Ensure correct dimensions
        first_four = x[:, :4]
        pos_feature = self.pos_fc1(first_four)
...
class TranPosSpaceScale(nn.Module):
    """
    Experiment: 2024070401 (Multi-scale data)
    """
    def __init__(self, dropout_rate = 0.1):
        super(TranPosSpaceScale, self).__init__()
        # New data, high-resolution time series
        self.transformer1 = TimeSeriesTransformer(seq_length=49)  # For columns 5-53
        self.transformer2 = TimeSeriesTransformer(seq_length=49)  # For columns 54-102
        # Old data, low-resolution time series
        self.transformer3 = TimeSeriesTransformer(seq_length=61)  # For columns 5-65
        self.transformer4 = TimeSeriesTransformer(seq_length=61)  # For columns 66-126

        self.fc1 = nn.Linear(128 + 128 + 500, 1024)  # Two transformer outputs and first 4 columns
        self.lkrelu = nn.LeakyReLU()
        self.dropout = nn.Dropout(dropout_rate)
        self.fc2 = nn.Linear(1024, 1024)
        self.fc3 = nn.Linear(1024, 512)
    ...
    def forward(self, x):
        ts_features1 = self.transformer1(x[:, 4:53])  # Feature extraction (cols 5-53)
        ts_features2 = self.transformer2(x[:, 53:102])  # Feature extraction (cols 54-102)
        ts_features3 = self.transformer3(x[:, 102:163])  # Feature extraction (cols 5-53)
        ts_features4 = self.transformer4(x[:, 163:])  # Feature extraction (cols 54-102)
        first_four = x[:, :4]  # First 4 columns
        pos_feature = self.pos_fc1(first_four)
        pos_feature = self.lkrelu(pos_feature)
        pos_feature = self.pos_fc2(pos_feature)
...
class LstmPosSpaceScale(nn.Module):
    """
    Experiment: 2024070402 (Multi-scale data + LSTM)
    2024070701 (Balanced data)
    2024071101 (Another balancing scheme)
    """
    def __init__(self, dropout_rate = 0.1):
        ...
        self.lstm3 = TimeSeriesLSTM(hidden_dim=64, seq_length=61)  # Output dim 64
        self.lstm5 = TimeSeriesLSTM(hidden_dim=64, seq_length=61)  # Output dim 64
        self.fc1 = nn.Linear(128 + 128 + 500, 1024)  # Transformer outputs and first 4 columns
        self.lkrelu = nn.LeakyReLU()
        self.dropout = nn.Dropout(dropout_rate)
        self.fc2 = nn.Linear(1024, 1024)
        self.fc3 = nn.Linear(1024, 512)
    ...
    def forward(self, x):
        ts_features1 = self.lstm1(x[:, 4:53].unsqueeze(-1))  # Ensure correct dimensions
        ts_features2 = self.lstm2(x[:, 53:102].unsqueeze(-1))  # Ensure correct dimensions
        ts_features3 = self.lstm3(x[:, 102:163].unsqueeze(-1))  # Ensure correct dimensions
        ts_features4 = self.lstm5(x[:, 163:].unsqueeze(-1))  # Ensure correct dimensions
        first_four = x[:, :4]  # First 4 columns
        pos_feature = self.pos_fc1(first_four)
        pos_feature = self.lkrelu(pos_feature)
        pos_feature = self.pos_fc2(pos_feature)
...
class KstmScalePosSpace(nn.Module):
    """
    Experiment: 2024070403 (LSTM + KAN + Multi-scale data)
    """
    def __init__(self, dropout_rate=0.1):
        ...
        # Low-resolution time series
        self.lstm3 = TimeSeriesLSTM(hidden_dim=64, seq_length=61)  # Output dim 64
        self.lstm4 = TimeSeriesLSTM(hidden_dim=64, seq_length=61)  # Output dim 64
        self.kan1 = KAN([4, 64, 500])
    ...
    def forward(self, x):
        ...
        ts_features2 = self.lstm2(x[:, 53:102].unsqueeze(-1))  # Ensure correct dimensions
        ts_features3 = self.lstm3(x[:, 102:163].unsqueeze(-1))  # Ensure correct dimensions
        ts_features4 = self.lstm4(x[:, 163:].unsqueeze(-1))  # Ensure correct dimensions
        first_four = x[:, :4]
        pos_feature = self.kan1(first_four)
        combined_features = torch.cat([ts_features1, ts_features2, ts_features3, ts_features4, pos_feature], dim=1)
        x = self.kan2(combined_features)
...
class AttenLstmPosSpaceScale(nn.Module):
    ...
    def __init__(self, dropout_rate=0.1):
        ...
        self.lstm3 = TimeSeriesAttnLSTM(hidden_dim=64, seq_length=61)  # Output dim 64
        self.lstm4 = TimeSeriesAttnLSTM(hidden_dim=64, seq_length=61)  # Output dim 64
        self.fc1 = nn.Linear(256 + 500, 1024)  # Total dim of 4 LSTM outputs and pos_feature
        self.lkrelu = nn.LeakyReLU()
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout_rate)
        self.fc2 = nn.Linear(1024, 1024)
...
class AttenKANLstmPosSpaceScale(nn.Module):
    """
    Experiment: 2024071802 (Added attention module, combined with KAN)
    """
    def __init__(self, dropout_rate=0.1):
